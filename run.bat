...
rem Require bundled portable JRE only (no system Java fallback)
set "JAVA_EXE=%SCRIPT_DIR%\jre\bin\javaw.exe"
if not exist "%JAVA_EXE%" set "JAVA_EXE=%SCRIPT_DIR%\jre\bin\java.exe"

if not exist "%JAVA_EXE%" (
  echo.
  echo ERROR: Bundled portable JRE not found at %SCRIPT_DIR%\jre\bin\java[w].exe
  echo Ensure installer\packages\jre-portable was included before packaging.
  exit /b 1
)
